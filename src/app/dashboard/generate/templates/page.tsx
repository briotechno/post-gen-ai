    ...
    <div className="backdrop-blur-xl rounded-xl">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        {templates.map(({ tag, action, text }) => (
          <div
            key={text}
            className="shadow-[0_0px_5px_rgba(0,0,0,0.1)] border border-white/10 rounded-xl p-4 flex flex-col justify-between transition-all hover:bg-white/5"
          >
            <div className="flex items-center justify-between mb-2">
              <span className="text-xs font-medium bg-white/10 px-2 py-0.5 rounded-full">
                {tag}
              </span>
              <button className="text-xs hover:text-white font-medium transition-all">
                {action}
              </button>
            </div>
